...
from codewiki.src.be.backend import TaskBackendBase, get_backend
...
class DocumentationGenerator:
    ...
    def __init__(self, config: Config, commit_id: str = None, backend: TaskBackendBase = None):
        self.config = config
        self.commit_id = commit_id
        self.graph_builder = DependencyGraphBuilder(config)
        self.backend: TaskBackendBase = backend or get_backend(config)
    ...
    def _backend_has_pending_tasks(self) -> bool:
        checker = getattr(self.backend, "has_pending_tasks", None)
        return bool(checker and checker())

    def _raise_backend_pending_tasks(self) -> None:
        raiser = getattr(self.backend, "raise_if_pending", None)
        if raiser:
            raiser()

    def _module_doc_exists(self, working_dir: str, module_name: str) -> bool:
        return self._resolve_child_docs_path(working_dir, module_name) is not None

    def _children_docs_ready(self, module_info: Dict[str, Any], working_dir: str) -> bool:
        children = module_info.get("children", {})
        if not isinstance(children, dict) or not children:
            return True
        return all(self._module_doc_exists(working_dir, child_name) for child_name in children)
    ...
    async def generate_module_documentation(self, components: Dict[str, Any], leaf_nodes: List[str]) -> str:
        ...
        if len(module_tree) > 0:
            for module_path, module_name in processing_order:
                try:
                    ...
                    if self.is_leaf_module(module_info):
                        ...
                        if not self._children_docs_ready(module_info, working_dir):
                            logger.info(
                                f"⏳ Skipping parent module until child docs are ready: {module_key}"
                            )
                            continue
            ...
            if self._backend_has_pending_tasks():
                self._raise_backend_pending_tasks()

            # Generate repo overview
            if self._top_level_docs_ready(file_manager.load_json(module_tree_path), working_dir):
                logger.info(f"📚 Generating repository overview")
                final_module_tree = await self.generate_parent_module_docs(
                    [], working_dir
                )
            else:
                logger.info("⏳ Skipping repository overview until top-level docs are ready")
        else:
    ...
    async def generate_parent_module_docs(self, module_path: List[str], 
        ...
        try:
            ...
            # Parse and save parent documentation. Some task results may contain
