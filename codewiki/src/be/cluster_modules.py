...
def cluster_modules(
    ...
    Args:
        completer: ``(prompt: str) -> str`` callable that writes/reads task files.
    """
    ...
    try:
        if "<GROUPED_COMPONENTS>" not in response or "</GROUPED_COMPONENTS>" not in response:
            logger.error(f"Invalid task result format - missing component tags: {response[:200]}...")
            return {}
        ...
        logger.error(f"Failed to parse task result: {e}. Response: {response[:200]}...")
    ...
    for module_name, module_info in module_tree.items():
        ...
        if len(current_module_path) < config.max_depth:
